...
We recommend that you use RStudio as your environment to make things easier.
...
So, to do this, we are going to take the data frame with our respective player's winning matches and filter out all losing players with a rank greater than 20 (Which in the case of tennis, would be a worse rank). 

Then, we use the group_by operation to group each data frame by year so that we can later plot our player's top 20 wins per yearly season. You can read more about the group_by operation in R via its documentation here:
...
Now, for each player's data frame, we need to add a Player attribute to help us distinguish which player came from which data frame. Then, we bind or combine the three data frames together.
...
After we have the groups, we can summarize these wins by using the n function to calculate the wins per surface for each player. 
...
We can then summarize using the n function to get all wins per slam, and add a Player attribute to help us distinguish which player came from which data frame. Then, we bind or combine the three data frames together.
...
The Hypothesis we will test is: Has Rafael Nadal and Novak Djokovic affected Roger Federer's career? And to what extent.


### Message

Before we think of a takeaway from this tutorial/analysis, let's just go back to the results of our analysis:
